/*! GalleriomJS (1.0) (C) Denis Mušić. WTFPL @license: https://en.wikipedia.org/wiki/WTFPL */
...
(function(global, galleryDOM) {
    ...
    Galleriom.prototype.createImages = function(sizes) {
        ...
        for ( ; i < length; ++i) {
            this.elAdjuster(this.galleryCollection[i], sizes);
            this.setClassAttr(this.galleryCollection[i], 'galleriom-img');
            // Prevent click event if preview is selected stimulate other waay
            if (!this.gadget.preview) this.addListeners('click', this.galleryCollection[i].parentNode, function(evt) { evt.preventDefault(); });
    ...
    Galleriom.prototype.createThumbsView = function() {
        ...
        if (this.gadget.thumbsView) {
            ...
            for (var i = 0, n = this.dimensions.quantity + 1; i < n; ++i) {
                thumb = document.createElement('div');
                // Add first thumb class selected-thumb
                this.setAttributes(thumb, { 'class': 'galleriom-thumbs-dotts', 'data-no': i });
                this.addListeners('click', thumb, function(evt) { that.thumbsToView(evt); });
                thumbsHolder.appendChild(thumb);
            }
        }
    ...
    Galleriom.prototype.markThumb = function(position) {
        ...
        if (markedEl) { markedEl.className = ''; }

        // Add class to clicked thumb
        if (elToMark[position]) { elToMark[position].className = 'selected-thumb'; }
    };
    ...
    Galleriom.prototype.grabber = function(el) {
        ...
        // get width; check if there is % sign; store data in constructor or get el.offsetWidth
        ...
        if (attrs['data-preview']) { this.gadget.preview = true; }
    };
